...
def load_experiments():
    ...
    for run in data:
        ...
        mono = run['parameters'].get('monotonicity', {})
        if isinstance(mono, dict) and mono.get('enabled'):
            # Count non-zero constraints
            constraints = mono.get('constraints', {})
            active_count = sum(1 for v in constraints.values() if v != 0)
            constraint_str = f"✅ Monotonic ({active_count})"
        else:
            constraint_str = "❌ None"
